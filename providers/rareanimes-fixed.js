...
const atob = (input) => {
  ...
  }                          // FIX #1: closing brace for for-loop body was missing
  return output;
};
...
const levenshtein = {
  get: function (s, t) {
    ...
    for (var i = 1; i <= n; i++) {
      ...
      }            // FIX #2: inner for-loop closing brace was missing
    }              // FIX #2: outer for-loop closing brace was missing
    return d[n][m];
  }                // FIX #2: get function closing brace was missing
};
  ...
  return fetchText(searchUrl).then(function (html) {
    ...
    if (!results.length) {
      $('a').each(function (_i, el) {
        ...
        }    // FIX #3: closing brace for if-block was missing
      });
    }        // FIX #3: closing brace for if (!results.length) was missing
  ...
  return fetchText(pageUrl).then(function (html) {
    ...
    $('a[href*="codedew.com"]').each(function (_i, el) {
      ...
      if (episode) {
        ...
        // FIX #4: only skip if a number was matched AND it doesn't match the target episode
        // Previously it skipped when no match was found at all (links with no episode label were dropped)
...
// -----------------------------------------------------------------------------
// FIX #6: Robust bracket-depth JS array extractor (replaces fragile regex)
// -----------------------------------------------------------------------------
function extractJsArray(html, varName) {
  // Find the start of the array assignment
  var startMarker = varName + ' = [';
  var idx = html.indexOf(startMarker);
  if (idx === -1) {
    // Also try const/let/var prefix without spaces after =
    var re = new RegExp('(?:let|const|var)\\s+' + varName + '\\s*=\\s*\\[');
    var m = html.match(re);
    if (!m) return [];
    idx = html.indexOf(m[0]);
  }

  // Walk forward to find the array open bracket
  var start = html.indexOf('[', idx);
  if (start === -1) return [];

  // Walk through the string tracking bracket depth, respecting strings
  var depth = 0;
  var i = start;
  var inStr = false;
  var strChar = '';
...
// -----------------------------------------------------------------------------
// FIX #5: Follow JS/meta redirects before scanning for playerSources
// Step 2+3: codedew redirect -> streambeta player -> extract worker URLs
// -----------------------------------------------------------------------------
function followRedirectPage(html, originalUrl) {
  // Check for meta-refresh redirect
  var metaMatch = html.match(/<meta[^>]+?(?:http-equiv=["']?refresh["']?[^>]+?content=["'][^;]*;\s*url=|url=\s*)["']?([^"'\s>]+)/i);
  if (metaMatch && metaMatch[1]) {
    var metaUrl = metaMatch[1];
    if (metaUrl.indexOf('http') !== 0) {
      // Relative URL — resolve against original
      metaUrl = new URL(metaUrl, originalUrl).href;
    }
    console.log('[RareAnimes] Following meta-refresh to: ' + metaUrl);
    return fetchText(metaUrl).then(function (nextHtml) { return nextHtml || html; });
  }

  // Check for JS window.location redirect
  var jsMatch = html.match(/(?:window\.location(?:\.href)?\s*=\s*|location\.replace\s*\(\s*)['"]([^'"]+)['"]/);
  if (jsMatch && jsMatch[1]) {
    var jsUrl = jsMatch[1];
    if (jsUrl.indexOf('http') !== 0) {
      jsUrl = new URL(jsUrl, originalUrl).href;
    }
    console.log('[RareAnimes] Following JS redirect to: ' + jsUrl);
    return fetchText(jsUrl).then(function (nextHtml) { return nextHtml || html; });
  }

  // No redirect found — return the original html as-is
  return Promise.resolve(html);
}

function extractStreamsFromHtml(html, episodeLabel) {
  var streams = [];

  var playerSources = extractJsArray(html, 'playerSources');
  playerSources.forEach(function (src) {
    var url = src.stream_url || src.url || src.file;
    if (url) {
      streams.push({
        name: 'RareAnimes - ' + (src.name || src.label || 'Server'),
        title: episodeLabel + ' - ' + (src.name || src.label || 'Server') + ' (Stream)',
        url: url,
  ...
  downloadSources.forEach(function (src) {
    ...
    if (url) {
      streams.push({
        name: 'RareAnimes - ' + (src.name || src.label || 'Server') + ' (DL)',
        title: episodeLabel + ' - ' + (src.name || src.label || 'Server') + ' (Download)',
        url: url,
...
function resolveStreamsFromRedirect(redirectUrl, episodeLabel) {
  return fetchText(redirectUrl).then(function (html) {
    ...
    // FIX #5: follow any intermediate redirect before scanning for video sources
    return followRedirectPage(html, redirectUrl).then(function (finalHtml) {
      var streams = extractStreamsFromHtml(finalHtml, episodeLabel);
    ...
    return findBestPage(title, isSeries ? season : null).then(function (pageUrl) {
      ...
      return findRedirectLinks(pageUrl, isSeries ? episode : null).then(function (redirects) {
        ...
        return Promise.all(promises).then(function (results) {
          ...
          // De-dupe across multiple redirect links
          var seen = {};
          var deduped = [];
          streams.forEach(function (s) {
            if (!seen[s.url]) {
